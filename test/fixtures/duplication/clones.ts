...
export function summarizeOrders(orders: { status: string; amount: number }[]): number {
  let total = 0;
  let count = 0;
...
export function summarizeRefunds(refunds: { status: string; amount: number }[]): number {
  let sum = 0;
  let seen = 0;
