 ...
 * the only per-function reference tool (lizard) measures cyclomatic complexity, which ranking
 * ignores and which lizard counts per case label rather than per case-labelled statement, so
 * lizard's values are not adopted. Ruby has no readily runnable open-source implementation of SonarSource
