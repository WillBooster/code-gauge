...
- Per-function cyclomatic complexity (McCabe, own body only), counted as NIST SP 500-235 defines it:
  ...
  file total over McCabe's components (every function, decisions outside functions, and the module
  body of a file that runs top-level code); an API-only metric that ranking and the regression gate
  ignore
