#!/usr/bin/env node
// Release-time pipeline for the prebuilt binaries, driven by @semantic-release/exec's prepareCmd:
// wait for the released commit's "Build Native" workflow run, download its artifacts, place the
...
async function prepare(newVersion) {
  ...
  // The binary distributions must carry the statically linked dependencies' attributions
  // (generated by build-native.yml's third-party-notices job) plus this project's own license files.
  ...
  // The main package's `workerd` export imports the WebAssembly module, so the release must never
  // publish without it.
  const wasmPath = path.join(artifactsDirPath, 'code-gauge-wasm32-wasip1', 'code-gauge.wasm');
  if (!existsSync(wasmPath) || statSync(wasmPath).size === 0) {
    throw new Error(`Missing or empty WebAssembly artifact: ${wasmPath}`);
  }
  copyFileSync(wasmPath, path.join(packageRoot, 'native', 'code-gauge.wasm'));
  copyFileSync(noticesPath, path.join(packageRoot, 'THIRD-PARTY-NOTICES.txt'));
